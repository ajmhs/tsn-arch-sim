...
void process_data(dds::sub::DataReader< ::ShapeTypeExtended> reader, unsigned int& samples_read)
{
    ...
    for (auto sample : samples) {
        if (sample.info().valid()) {
            std::cout << "Read a video control sample (\"" << sample.data().color() << "\"), count: " << ++samples_read << std::endl;
        } else {
...
void run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    //dds::core::QosProvider qos_provider("video_qos_profiles.xml");
    ...
    dds::sub::DataReader< ::ShapeTypeExtended> reader(subscriber, topic, qos_provider.datareader_qos("tsn_library::video_profile"));
