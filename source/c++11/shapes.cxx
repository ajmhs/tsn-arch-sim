...
ShapeType::ShapeType (const std::string& color_,int32_t x_,int32_t y_,int32_t shapesize_):
    m_color_(color_), 
    m_x_(x_), 
    m_y_(y_), 
    m_shapesize_(shapesize_) {
}

void ShapeType::swap(ShapeType& other_)  noexcept 
{
    using std::swap;
...
ShapeTypeExtended::ShapeTypeExtended (const std::string& color_,int32_t x_,int32_t y_,int32_t shapesize_,const ::ShapeFillKind& fillKind_,float angle_) :
    ::ShapeType(color_, x_, y_, shapesize_), 
    m_fillKind_(fillKind_), 
    m_angle_(angle_) {
}

void ShapeTypeExtended::swap(ShapeTypeExtended& other_)  noexcept 
{
    using std::swap;
...
bool ShapeTypeExtended::operator == (const ShapeTypeExtended& other_) const {
    ...
    if (std::fabs(m_angle_ - other_.m_angle_) > std::numeric_limits< float>::epsilon()
    && !(std::fabs(m_angle_ - other_.m_angle_) < (std::numeric_limits< float>::min)())) {
        return false;
    }
    return true;
}
